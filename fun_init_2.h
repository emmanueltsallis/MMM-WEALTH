...
class1=SEARCH_CND("class_id",1);   // Top 0.01%
class2=SEARCH_CND("class_id",2);   // Next 0.09% (class 1 + 2 = top 0.1%)
class3=SEARCH_CND("class_id",3);   // Next 0.9% (class 1 + 2 + 3 = top 1%)
class4=SEARCH_CND("class_id",4);   // Next 9% (class 1 + 2 + 3 + 4 = top 10%)
class5=SEARCH_CND("class_id",5);   // Middle 40% (class 1 + 2 + 3 + 4 + 5 = top 50%)
class6=SEARCH_CND("class_id",6);   // Bottom 50% (class 1 + 2 + 3 + 4 + 5 + 6 = 100%)
...
v[226]+=(v[193]-v[194]);											//total demand loans
	...
	v[225]=VS(capital, "sector_capital_duration");
	v[226]=0;
	...
	CYCLE(cur, "CLASSES")
	{
        ...
        if (VS(cur, "class_id") == 1) {
          WRITES(cur, "class_initial_propensity_import", 0); // Force Class 1 initial propensity to import to 0
          LOG("\nClass 1 initial propensity to import overridden to 0.");
        }
		...
		for(i=1;i<=v[0]+1;i++) 	WRITELLS(cur, "Class_Nominal_Disposable_Income", v[245], 0, 1);
		for(i=1;i<=v[0]+1;i++)	WRITELLS(cur, "Class_Real_Disposable_Income", v[245]/v[13], 0, 1);
		for(i=1;i<=v[0]+1;i++)	WRITELLS(cur, "Class_Debt_Rate", 0, 0, 1);
	}
