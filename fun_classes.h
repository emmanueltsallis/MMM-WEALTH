...
*/
	...
	//v[5] = v[0] + v[1] + v[2] * (v[3] - v[4]) + v[6];  // WITH profit share component
	v[5]=v[0]+v[1]+v[6];								 // WITHOUT profit share component
RESULT(max(v[5],0))
...
EQUATION("Class_Profits")
/*
Class volume of profits
*/
	v[0] = VS(country,"Country_Distributed_Profits");		// Total distributed profits
	v[1] = V("class_profit_share");							// wage share of each class (parameter)
	
	v[2] = v[0] * v[1]; // profits = distributed profits * profit share
RESULT(v[2])
	
	
	
EQUATION("Class_Wages")
/*
Class volume of wages
*/
	v[0] = VS(country,"Country_Total_Wages");				// Total wages
	v[1] = V("class_wage_share");							//wage share of each class (parameter)
...
*/
	...
	w_T = aggregate_threshold_param / total_population; // Uniform per-household threshold
